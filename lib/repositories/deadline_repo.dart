...
class DeadlineRepo implements DeadlineRepository {
  ...
  Future<void> create(Deadline deadline) async {
    ...
    if (null != _supabaseClient.auth.currentSession) {
      ...
      final List<Map<String, dynamic>> response =
          await _supabaseClient.from("deadlines").insert(deadlineEntity).select("id");
      id = response.last["id"];
      if (null == id) {
  ...
  Future<void> update(Deadline deadline) async {
    ...
    if (null != _supabaseClient.auth.currentSession) {
      Map<String, dynamic> deadlineEntity = deadline.toEntity();
      final List<Map<String, dynamic>> response =
          await _supabaseClient.from("deadlines").update(deadlineEntity).select("id");
  ...
  // Search + Most Recent
  @override
  Future<List<Deadline>> search({required String searchString}) async => await _isarClient.deadlines
      .filter()
  ...
  Future<List<Deadline>> mostRecent({int limit = 50}) async =>
      await _isarClient.deadlines.where().sortByLastUpdatedDesc().limit(limit).findAll();
  ...
  @override
  Future<List<Deadline>> getRepoList({int limit = 50, int offset = 0}) => _isarClient.deadlines
      .where()
  ...
  Future<List<Deadline>> getRepoListBy(
      {int limit = 50, int offset = 0, required SortableView<Deadline> sorter}) async {
  ...
  Future<List<int>> getDeleteIds() async =>
      _isarClient.deadlines.where().toDeleteEqualTo(true).idProperty().findAll();
